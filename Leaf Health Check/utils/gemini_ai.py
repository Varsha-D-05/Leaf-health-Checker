...
class GeminiAIEngine:
    ...
    def chat(self, message: str) -> str:
        ...
        try:
            # Build a simple conversational prompt including history
            history_text = "\n".join(
                [f"User: {entry['parts']}" if entry['role'] == 'user' else f"Assistant: {entry['parts']}" for entry in self.chat_history]
            )
